...
float values[11];
float minThreshold[] = {660, 600, 540, 590, 550, 670, 600, 590, 670};  // Threshold array to define the minimum valid reading per sensor
...
char categorizeRead(float maxValues[]){
  ...
  if (maxValues[3] > threshold_half_f2a && maxValues[4] > threshold_half_f2b) {                   // Check if middle finger is over halfway bent
    ...
    if (maxValues[1] > threshold_half_f1a){                                                     //check if pointer finger tip is over halfway bent
      ...
      if (maxValues[5] > threshold_half_f3a) {                                                //check if ring finger is over halfway bent, H U V R
...
void printLetter(char letter, int confidence){
  Serial.print("\n");
...
void loop(){
  ...
  while (values[0] > minThreshold[0] || values[1] > minThreshold[1] || values[2] > minThreshold[2] || values[3] > minThreshold[3] || values[4] > minThreshold[4] || values[5] > minThreshold[5] || values[6] > minThreshold[6] || values[7] > minThreshold[7] || values[8] > minThreshold[8]){
    values[0] = analogRead(A11);    //thumb
    values[1] = analogRead(A8);     //pointer tip
    values[2] = analogRead(A15);    //pointer base
    values[3] = analogRead(A7);     //middle tip
  ...
  int j = sizeof(count_array) / sizeof(count_array[0]);
  
  if ((int(maxVal)) != 0){
    // Check for specific letters
    if (count_array[7] > 0) {
      printLetter(letter_array[7], (count_array[7]/j)*100);
    } else if (count_array[9] > 0) {
      printLetter(letter_array[9], (count_array[9]/j)*100);
    } else if (count_array[15] > 0) {
      printLetter(letter_array[15], (count_array[15]/j)*100);
    } else if (count_array[6] > 0) {
      printLetter(letter_array[6], (count_array[6]/j)*100);
    } else {
        // Find the letter with the maximum count
        for (int i = 0; i < 27; i++) {
            if (count_array[i] == maxVal) {
              printLetter(letter_array[i], (count_array[i]/j)*100);
            }
        }
    }
  } else {
